...
class HiveEventHandler:
    def __init__(
        self, atlas_hostname: str, atlas_port: int, oidc_token_manager: OidcTokenManager
    ):
        self.__atlas_hostname = atlas_hostname
        self.__atlas_port = atlas_port
        self.__oidc_token_manager = oidc_token_manager
        self.__atlas_client = Atlas(
            host=self.__atlas_hostname,
            port=self.__atlas_port,
    ...
    def handle_create_table_event(self, event_msg: str):
        ...
        try:
            ...
            db_description = ""
            self.__hive_db_manager.create_entity(
                db_name, cluster_name, db_description, owner=owner
            )
        except Exception as e:
            my_logger.exception(
                f"Can't create hive database with name: {db_name}\n {e}"
            )
            return False
        ...
        try:
            ...
            self.__hive_table_manager.create_entity(
                table_name,
                db_qualified_name,
                table_description,
                owner=owner,
        ...
        try:
            ...
            for col in cols:
                ...
                self.__hive_column_manager.create_entity(
                    col_name, col_type, table_qualified_name, col_description
                )
        except Exception as e:
            my_logger.exception(
    ...
    def handle_drop_table_event(self, event_msg: str, purge=True):
        ...
        try:
            guid = self.__finder.get_guid_by_qualified_name(
                "hive_table", qualified_name
            )
        except Exception as e:
            my_logger.exception(
                f"Can't find guid for given entity qualified_name {qualified_name} \n{e}"
            )
            return False
        else:
